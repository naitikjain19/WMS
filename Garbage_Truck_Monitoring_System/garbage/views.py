...
def post_create_driver(request):
    ...
    # name = database.child('users').child(id).child('details').child('name').get().val()
    welcome=database.child('Driver').child(mobileNo).child('name').get().val()
    return render(request,'welcome.html', {'e':welcome})
...
# --------------------------Driver
    ...
    return render(request,'Check.html',{'comb_lis':comb_lis,'comb_lis_bin':comb_lis_bin,'e':"User"})

def check_queries(request):
    name= database.child('Citizen').child('name').get().val()
    address= database.child('Citizen').child('address').get().val()
    citizen_id= database.child('Citizen').child('citizen_id').get().val()
    date= database.child('Citizen').child('date').get().val()
    query= database.child('Citizen').child('query').get().val()
    ...
    # query = []
    # date = []
    # for key,value in citizen_details.items():
    ...
    return render(request,'checkQueries.html',{
        "name":name,
        "address":address,
...
def create_distance_matrix(data):
    ...
    API_key = 'AIzaSyAO2z9WtUvgop98jVAbWuEXdNGaukGWRyU'
...
def generate_routes(request):
    ...
    print("Create the routing index manager.", len(datap['distance_matrix']), datap['num_vehicles'], datap['depot'])
...
def updateFeedback(request):
    ...
    return render(request,'welcome.html', {'e':"User"})
